...
echo "🔧 [2/4] Applying Nginx Reverse Proxy for Shodh Labs (Website 1)..."
bash configure-nginx-shodhlabs.sh

echo "🐍 [3/4] Ensuring Shodh Labs is running with 4 Gunicorn workers..."
if [ -d "/home/extre0101/SHODH_LABS/venv" ]; then
    PCH_DIR="/home/extre0101/SHODH_LABS"
elif [ -d "/home/pi/SHODH_LABS/venv" ]; then
    PCH_DIR="/home/pi/SHODH_LABS"
else
    PCH_DIR=""
fi

if [ -n "$PCH_DIR" ] && command -v pm2 &> /dev/null; then
    pm2 stop shodhlabs 2>/dev/null || true
    ...
    pm2 start "$PCH_DIR/venv/bin/gunicorn" \
        ...
        app:app
    pm2 save
    echo "🟢 Shodh Labs is active under PM2 with 4 workers!"
fi
...
echo "• Shodh Labs (Site 1):   https://extre0101.github.io/homeserver/?go=site1"
